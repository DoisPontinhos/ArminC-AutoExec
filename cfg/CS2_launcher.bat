@(set ^ "0=%~f0" -d ') & start "@" conhost powershell -nop -c iex(gc -lit $env:0 -raw) & exit /b ');.{
<#
  Counter-Strike 2 launcher - AveYo, 2025.01.13 
  ...
  + optionally force specific video settings at every launch; parses removed launch options like -refresh -r -sdl_displayindex
  2025.01.13: fixed one-time initialization of SetRes library
#>
...
# "setting.videocfg_fsr_detail"                      = "-1"            #  0     FSR quality: 2 | balanced: 3 | minecraft: 4
...
# "trusted_launch"                                   = "1"            #  1     trusted launch tracking
  ...
  "r_show_build_info"                                = "0"            #  1     build info is a must when reporting issues
}
...
$extra_launch_options+= '-noreflex'
$extra_launch_options+= '+exec meow.cfg'
...
$STEAM = resolve-path (gpv "HKCU:\SOFTWARE\Valve\Steam" SteamPath); $GAME = ''
...
if ($unify_cfg -eq 0) {
  ...
  elseif ($USRLOCALCSGO_Ub) {
    ...
    if ($GAME) { robocopy "$USRLOCALCSGO_U\cfg/" "$USRCLOUD\$APPID\local\cfg/" *.txt *.cfg *.vcfg /XO >'' }
    if (ps "Steam" -ea 0) {
...
if ($unify_cfg -eq 1) { 
  ...
  elseif ($USRLOCALCSGO_Ub -and $USRLOCALCSGO_U -eq "$GAME") {
    ...
    if ($GAME) { robocopy "$USRCLOUD\$APPID\local\cfg/" "$GAME\cfg/" *.vcfg *video*.txt /XO >'' }
    if ($GAME) { robocopy "$USRCLOUD\$APPID\local/" "$GAME/" socache.dt /XO >'' }
  }  
  else {
    $USRLOCAL = "$GAME"
    [Environment]::SetEnvironmentVariable("USRLOCALCSGO","$GAME",0)
    [Environment]::SetEnvironmentVariable("USRLOCALCSGO","$GAME",1)
    if ($USRLOCALCSGO_Ub) {
      if ($GAME) { robocopy "$USRLOCALCSGO_U\cfg/" "$GAME\cfg/" *.vcfg *video*.txt /XO >'' }
      if ($GAME) { robocopy "$USRLOCALCSGO_U/" "$GAME/" socache.dt /XO >'' }
    }
    if (ps "Steam" -ea 0) {
      write-host " will try closing Steam to refresh USRLOCAL gamevar" -fore Yellow
      start "$STEAM\Steam.exe" -args '-shutdown' -wait; sleep 5
...
if ($GAME -and -not (test-path "$GAME\cfg\autoexec.cfg")) { sc "$GAME\cfg\autoexec.cfg" "" }
...
if ((gi $path1 -force -ea 0).VersionInfo.FileVersion -ne $version1) { del $path1 -force -ea 0 } ; if (-not (test-path $path1)) {
  ...
  sc "$env:APPDATA\AveYo\$library1.cs" $(($MyInvocation.MyCommand.Definition -split '<#[:]LIBRARY1[:].*')[1])
...
if ($unify_cfg -gt 0 -and $USRLOCAL -ne "$USRCLOUD\$APPID\local") {
  if ($GAME) { robocopy "$USRLOCAL\cfg/" "$USRCLOUD\$APPID\local\cfg/" *convars.vcfg *slot0.vcfg *video*.txt /XO >'' }
}
...
if (test-path $appmanifest) {
  ...
  if ($GAME) { sc $appmanifest $blank -force }
}
...
$window = @("-coop_fullscreen", "-fullscreen")[$exclusive -ge 1]
$video_options = "$window -width $width -height $height -refresh $refresh -sdl_displayindex $sdl_idx "
write-host " $video_options" -fore Green
...
if ($do_not_set_desktop_res_to_match_game -le 0 -and -not $sameres) {
  ...
  if ($GAME) { while ($null -eq ($wait = ps $APPNAME -ea 0)) { sleep -m 250 } }
  ...
  if ($GAME) { while (-not $wait.HasExited) { sleep 5 } }
